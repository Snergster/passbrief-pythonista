#!/usr/bin/env python3
"""
SR22T Departure/Arrival Briefing Tool for Pythonista iOS
Version 31.0 - WORKFLOW EDITION with Complete Feature Set

OVERVIEW:
Comprehensive flight briefing tool for Cirrus SR22T operations with Garmin Pilot integration,
automatic airport/runway data fetching, accurate magnetic variation handling, and performance
calculations based on embedded POH data.

CURRENT FEATURES:
✅ Garmin Pilot Integration - PDF flight briefing parsing
✅ NOAA Weather Integration - Real-time METAR with automatic unit conversion  
✅ Airport/Runway Database - OurAirports integration with automatic data lookup
✅ Accurate Magnetic Variation - Regional approximation with user verification
✅ Performance Calculations - Takeoff/landing distances, climb gradients (91/120 KIAS)
✅ Wind Component Analysis - Headwind/crosswind/tailwind calculations
✅ Wind-Based Warnings - Alerts for high winds (20+ kt) or crosswinds (10+ kt)
✅ Interactive UI - Multiple input workflows with diagnostic mode
✅ Aviation Safety Focus - Proper magnetic/true heading handling

RECENT IMPROVEMENTS (v30 → v31):
🔧 Implemented all roadmap features: SID, CAPS, FlavorText, ChatGPT analysis
🔧 Added sequential workflow system with enhanced user experience
🔧 Complete Garmin Pilot integration replacing ForeFlight
🔧 Professional briefing format with comprehensive safety checks
🔧 Advanced route analysis and weather integration
🔧 Production-ready implementation with full testing suite

IMPLEMENTED FEATURES (v31.0):
✅ 1. SID Departure Check Functionality
   - Integration with departure procedures database
   - Check runway vs. available SIDs compatibility
   - Display applicable departure procedures with restrictions

✅ 2. CAPS Altitudes into Takeoff Briefing  
   - Add Cirrus Airframe Parachute System altitude awareness
   - Display minimum deployment altitude for departure area
   - Include CAPS considerations in emergency brief

✅ 3. Flavor Text for Takeoff Briefing
   - Enhanced readability and pilot-friendly language
   - Professional aviation briefing format
   - Improved visual formatting and contextual narrative

✅ 4. ChatGPT Support for Garmin Pilot Flight Briefing Analysis
   - AI-powered route analysis and insights
   - Weather trend analysis along route
   - Alternate recommendations and route optimization

VERSION HISTORY:
v31.0 - WORKFLOW EDITION - Complete feature implementation with sequential workflow
v30.0 - Accurate magnetic heading system with wind warnings
v29.0 - Complete application with performance calculations (patched from v27)
v27.0 - Complete reference implementation

DEPENDENCIES:
- requests (METAR/airport data)
- pdf parsing libraries (Garmin Pilot briefing parsing) 
- datetime, math, json, os, glob (standard library)
- requests (for NOAA WMM API - automatic magnetic variation calculation)

USAGE:
python3 sr22t_briefing_v30.py

AUTOMATIC MAGNETIC VARIATION:
- Uses NOAA World Magnetic Model API (WMM2025) for precise calculations
- No installation required - works automatically with internet connection
- Falls back to manual entry or regional approximation if offline
"""
...
# ============================================================================
# AIRPORT MANAGER WITH AUTOMATIC RUNWAY DATA
# ============================================================================

# ============================================================================
# AIRPORT MANAGER - MAGNETIC VARIATION SAFETY CRITICAL COMPONENT
# ============================================================================
"""
WHY MAGNETIC VARIATION MATTERS FOR AVIATION SAFETY:

CRITICAL BUG FIXED: OurAirports database provides TRUE headings, but METAR weather 
provides MAGNETIC wind directions. Mixing these reference systems causes incorrect 
wind component calculations, potentially leading to:
- Wrong runway performance calculations
- Inadequate runway length assessments  
- Incorrect crosswind component calculations
- Poor decision-making for runway selection
...
# ============================================================================
# PERFORMANCE CALCULATOR - AVIATION SAFETY CRITICAL COMPONENT
# ============================================================================
"""
...
# ============================================================================
# FLAVOR TEXT MANAGER (TAKEOFF BRIEFING ENHANCEMENT)
# ============================================================================
...
class ChatGPTAnalysisManager:
    ...
    def _generate_pilot_hazard_analysis(self, flight_plan_data, operation, airport_data, weather_data, results):
        ...
        prompt = f"""You are an aviation safety specialist analyzing a comprehensive Garmin Pilot flight briefing with embedded route and weather data. Provide a detailed pilot hazard analysis.
...
CURRENT CONDITIONS:
- Airport: {airport_data['icao']} ({airport_data['name']})
- Runway: {airport_data.get('runway_heading', 'Unknown')}° magnetic  
- Current Weather: {weather_data['temp_c']}°C, {weather_data['altimeter']} inHg, Wind {weather_data['wind_dir']}°/{weather_data['wind_speed']}kt
- Density Altitude: {results['density_altitude']} ft
- Wind Components: {results['wind_components']['headwind']}kt headwind, {results['wind_components']['crosswind']}kt crosswind
...
Format as structured analysis with specific waypoints, altitudes, times, and quantified impacts."""
...
- Airport: {airport_data['icao']} ({airport_data['name']})
- Runway: {airport_data.get('runway_heading', 'Unknown')}° magnetic
- Weather: {weather_data['temp_c']}°C, {weather_data['altimeter']} inHg, Wind {weather_data['wind_dir']}°/{weather_data['wind_speed']}kt
- Density Altitude: {results['density_altitude']} ft
- Wind Components: {results['wind_components']['headwind']}kt headwind, {results['wind_components']['crosswind']}kt crosswind
"""
        
        prompt = f"""You are an aviation safety specialist. Analyze the following flight conditions and provide a concise pilot hazard analysis focused on actionable safety information for {scope_description}.
...
Format as a single paragraph starting with "Hazard Analysis:" and focus on practical, actionable safety information. If no significant hazards are identified, state that clearly."""
    ...
    def _generate_departure_passenger_brief(self, flight_plan_data, airport_data, weather_data, results):
        ...
        prompt = f"""Create a friendly, conversational passenger briefing script for departure. The pilot will read this to passengers before takeoff.

FLIGHT DETAILS:
- Route: {route_desc}
- Flight Time: {flight_time}
- Current Weather: {weather_data['temp_c']}°C, Wind {weather_data['wind_dir']}°/{weather_data['wind_speed']}kt
...
Start with "Here's what to expect for our flight:" and keep it conversational, informative, and reassuring. Use specific details from the route analysis to make it feel personalized to this flight."""
    ...
    def _generate_arrival_passenger_brief(self, flight_plan_data, airport_data, weather_data, results):
        ...
        prompt = f"""Create a brief, professional passenger script for arrival phase at {airport_data['icao']}.

CURRENT CONDITIONS:
- Approaching: {airport_data['icao']} ({airport_data['name']})
- Weather: {weather_data['temp_c']}°C, Wind {weather_data['wind_dir']}°/{weather_data['wind_speed']}kt
- Wind Components: {results['wind_components']['headwind']}kt headwind, {results['wind_components']['crosswind']}kt crosswind
...
Start with "We should be landing at {airport_data['name']} in about 12 minutes..." and keep it brief, professional but friendly, and reassuring."""
    ...
    def _filter_relevant_notams(self, notams_data, operation, airport_data):
        ...
        prompt = f"""Filter the following NOTAMs to show only information relevant to a {operation} operation at {airport_data['icao']}.

Operation Details:
- Type: {operation} 
- Airport: {airport_data['icao']}
- Time: {current_time}
- Runway: Runway {airport_data.get('runway_heading', 'Unknown')}°
...
Keep it concise and focused on what the pilot actually needs to know."""
